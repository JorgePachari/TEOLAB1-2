...
public:
    // Constructor para inicializar al paciente
    Paciente(int numAseg, const std::string& nom, int ed, const std::string& dir)
        : numeroAsegurado(numAseg), nombre(nom), edad(ed), direccion(dir) {}
...
public:
    // Constructor para inicializar al médico
    Medico(const std::string& nom, const std::string& esp)
...
public:
    // Constructor para inicializar la cita médica
    CitaMedica(const std::string& fh, const std::string& ta, const Paciente& pac, const Medico& med, const std::string& ec)
        : fechaHora(fh), tipoAtencion(ta), paciente(pac), medico(med), estadoCita(ec) {}
...
int main() {
    // Crear un paciente usando el constructor
    Paciente paciente1(123456, "Juan Perez", 30, "Calle Falsa 123");

    // Crear un médico usando el constructor
    Medico medico1("Dr. Garcia", "Medicina General");

    // Crear una cita médica usando el constructor
